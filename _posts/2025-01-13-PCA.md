...
If our \\(p\\) random variables, \\(\boldsymbol{x_1}, \dots, \boldsymbol{x_p}\\), look like
...
If we scale \\(u\\) by a nonzero scalar \\(a \in \mathbb R\\), we don't get a linear combination that is different in a way that we care about. If \\(u' =au\\), then the sample variance of \\(\boldsymbol{x_{u'}}\\) is the sample variance of \\(\boldsymbol{x_u}\\) multiplied by a factor of $$a^2:$$
...
The video below shows an animated demonstration.
